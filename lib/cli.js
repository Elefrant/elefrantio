...
function checkVersion() {
	...
	loadPackageJson(path.join(process.cwd(), 'node_modules/elefrantio/package.json'), function (err, data) {
	...
	npm.load({
		...
		npm.commands.outdated('elefrantio', true, function (err, list) {
			...
			if (latest < cliVersion) {
				console.log(chalk.yellow('    global elefrantio is out of date'));
				console.log('    Current: ' + cliVersion + ' Latest: ' + latest);
			} else {
				console.log(chalk.green('    global elefrantio at latest version:'), cliVersion);
			}
		});
	});
}
...
function requiresRoot(callback) {
	loadPackageJson(path.join(process.cwd(), 'package.json'), function (err, data) {
...
exports.publish = function () {
	...
	loadPackageJson(path.join('.', 'package.json'), function (err, pck) {
...
exports.init = function (name, options) {
	...
	shell.exec('git clone -b ' + source, function (code, output) {
		...
		loadPackageJson(path.join(name, '/package.json'), function (err, data) {
			if (err) {
			...
			var logoTxt = 'logo.txt';
			if (supportsColor) {
				logoTxt = 'logo_color.txt';
			}

			fs.readFile(__dirname + '/templates/' + logoTxt, function (err, data) {
...
exports.install = function (module, options) {
	requiresRoot(function () {
		...
		loadPackageJson(path.join('.', 'node_modules/elefrantio/package.json'), function (err, data) {
	...
	npm.load({
		...
		fs.readdir(packages, function (err, files) {
			...
			files.forEach(function (file) {
				...
				loadPackageJson(path.join(pkgPath, 'package.json'), function (err, data) {
					if (err || !data.engines.elefrant) return;
...
exports.pkg = function (name, options) {
	requiresRoot(function () {
		if (options.delete) {
			console.log(chalk.yellow('Removing component:'), name);
...
exports.action = function (name, options) {
	requiresRoot(function () {
		if (options.delete) {
			console.log(chalk.yellow('Removing action:'), name);
			shell.rm('-rf', './actions/' + name);
		} else {
			ensureEmpty('./actions/' + name, options.force, function () {
				require('./scaffold.js').actions(name, options);
			});
		}
	});
};