...
function checkVersion() {
	console.log();
	console.log('    checking elefrantio and global elefrant-cli versions');
	...
	npm.load({
		...
		npm.commands.outdated('elefrant-cli', true, function (err, list) {
			if (err) {
				console.log(chalk.red('Error: npm outdated failed'));
				return console.error(err);
			}
			...
			if (latest < cliVersion) {
				console.log(chalk.yellow('    elefrant-cli is out of date'));
				console.log('    Current: ' + cliVersion + ' Latest: ' + latest);
			} else {
				console.log(chalk.green('    elefrant-cli at latest version:'), cliVersion);
			}
		});
	});
}
...
function mongoConnect(env, callback) {
	...
	loadPackageJson('./package.json', function (err, data) {
		...
		var path = (data.elefrantVersion < '0.4.0' ? '/server' : '') + '/config/env/' + env + '.js';
...
function publishPackage(data, callback) {
	...
	var options = {
		uri: uri + 'api/v0.1/packages/publish',
		method: 'POST',
...
exports.search = function (keywords) {
	...
	var options = {
		uri: uri + 'api/v0.1/packages/search?q=' + keywords,
		method: 'GET'
	};
...
function getPackage(name, callback) {
	...
	var options = {
		uri: 'https://elefrant.com/api/components/' + name,
		method: 'GET'
	};
...
exports.status = function (options) {
	requiresRoot(function () {
		...
		loadPackageJson('./package.json', function (err, data) {
			...
			console.log('    ELEFRANT VERSION: ' + data.elefrantVersion);
			console.log();
