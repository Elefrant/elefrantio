...
Elefrant.prototype.init = function (callback) {
	...
	if(!path.existsSync(options.configPath)) {
		elefrant.log('error', 'Ouh! Couldn\'t find config path.');
	}
	if(!path.existsSync(options.actionPath)) {
		elefrant.log('error', 'Ouh! Couldn\'t find actions path.');
	}
	if(!path.existsSync(options.componentPath)) {
	...
	elefrant.name = elefrant.config.server.name = elefrant.config.server.name || elefrant.name;
