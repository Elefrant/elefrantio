		...
		if (err) {
			...
			component(Elefrant.Singleton, options.componentPath).then(function (components) {
				...
				.then(function (server) {
					Elefrant.Singleton.server = server;
					Elefrant.Singleton.register('server', server);

					return actions(Elefrant.Singleton, options.actionPath);
				})
				.then(function (actions) {
					console.log(middleware);
					if (_.isEmpty(actions)) {
						Elefrant.Singleton.log('warn', 'Couldn\'t find any action.');
					}

					return routes(Elefrant.Singleton, options.actionPath, actions);
				})
				.then(function (routes) {
					if (_.isEmpty(routes)) {
						Elefrant.Singleton.log('warn', 'Couldn\'t find any route.');
					}
					Elefrant.Singleton.register('routes', routes);

					Elefrant.Singleton.server.listen(Elefrant.Singleton.config.server.port, Elefrant.Singleton.config.server.host, function onListening() {
						callback(null, Elefrant.Singleton.server);
					});

					// console.log(Elefrant.Singleton.list);
				})
				.fail(function (err) {
					callback(err);
				})
