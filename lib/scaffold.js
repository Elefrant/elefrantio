...
function buildDirectoryStructureAction() {
	...
	mkdir(path, function () {
		write(path + '/action.js', readTemplate('action.js'));
	});

	mkdir(path + '/config', function () {
		write(path + '/config/' + name + '.js', readTemplate('/config/config.js'));
	});

	mkdir(path + '/controllers', function () {
		write(path + '/v1/' + name + '.js', readTemplate('/controllers/v1/controller.js'));
	});

	mkdir(path + '/models', function () {
		write(path + '/' + data.modelName + '.js', readTemplate('/models/model.js'));
	});

	mkdir(path + '/routes', function () {
		write(path + '/' + name + '.js', readTemplate('/routes/route.js'));
	});

	mkdir(path + '/test', function () {
		write(path + '/test/' + name + '.js', readTemplate('/test/test.js'));
	});
}
...
exports.actions = function (name, options) {
	...
	data = {
		pkgName: name.toLowerCase(),
		modelName: format.capitalize(this.pkgName),
		name: camelName,
		author: (typeof options.author === 'string' ? options.author : 'elefrant scaffold'),
		version: pkg.version
	};
